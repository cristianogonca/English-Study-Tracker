  ...
  while (diaCorrente < duracaoDias) {
    ...
    if (diasEstudo.includes(diaSemanaAtual)) {
      ...
      if (cicloDia === 1) {
        ...
        tarefas.push(criarTarefa('Academic Listening', 'Listen to lectures or documentaries', TipoConteudo.LISTENING, nivel, 30, 1));
        tarefas.push(criarTarefa('Critical Analysis', 'Analyze arguments and perspectives', TipoConteudo.LISTENING, nivel, 20, 2));
        tarefas.push(criarTarefa('Note Organization', 'Create structured notes', TipoConteudo.LISTENING, nivel, 10, 3));
        tituloSemana = `Week ${semana} - Academic Listening`;
      } else if (cicloDia === 4) {
        tarefas.push(criarTarefa('Complex Text', 'Read academic or professional text', TipoConteudo.READING, nivel, 30, 1));
        tarefas.push(criarTarefa('Deep Analysis', 'Analyze style, tone, and arguments', TipoConteudo.READING, nivel, 20, 2));
        tarefas.push(criarTarefa('Research Notes', 'Document insights and references', TipoConteudo.READING, nivel, 10, 3));
        tituloSemana = `Week ${semana} - Critical Reading`;
      } else if (cicloDia === 5) {
        tarefas.push(criarTarefa('Presentation Prep', 'Prepare structured presentation', TipoConteudo.SPEAKING, nivel, 20, 1));
        tarefas.push(criarTarefa('Advanced Speaking', 'Record detailed presentation or debate', TipoConteudo.SPEAKING, nivel, 30, 2));
        tarefas.push(criarTarefa('Professional Feedback', 'Self-evaluate like a professional', TipoConteudo.SPEAKING, nivel, 10, 3));
        tituloSemana = `Week ${semana} - Professional Speaking`;
      } else {
        tarefas.push(criarTarefa('Research & Outline', 'Research topic and create detailed outline', TipoConteudo.WRITING, nivel, 20, 1));
        tarefas.push(criarTarefa('Academic Writing', 'Write academic essay or professional article', TipoConteudo.WRITING, nivel, 30, 2));
        tarefas.push(criarTarefa('Peer Review', 'Review with critical eye', TipoConteudo.WRITING, nivel, 10, 3));
